...
# ------------------------------ 
# Pseudo 3-Channel training helpers 
# ------------------------------ 
...
def _enable_ultralytics_pseudo_rgb_dataloader() -> None: 
    """Monkey-patch Ultralytics dataset image loading to return 'Pseudo 3-channel' grayscale. 
    ...
    try: 
        ...
        raise RuntimeError(f"无法导入 Ultralytics 数据集模块以启用伪三通道训练: {e}") 
    ...
    def load_image_pseudo_rgb(self, i, rect_mode=False):  # type: ignore[override] 
        import cv2 
        im, f, fn = orig_load_image(self, i, rect_mode=rect_mode) 
        if im is None: 
            return im, f, fn 
        # 无论读进来的是什么，都先强制转成单通道灰度，然后再复制成 3 通道 (R=G=B)
        if im.ndim == 3 and im.shape[2] == 3: 
            gray = cv2.cvtColor(im, cv2.COLOR_BGR2GRAY) 
            im = cv2.cvtColor(gray, cv2.COLOR_GRAY2BGR)  # 变成伪三通道
        elif im.ndim == 2:
            im = cv2.cvtColor(im, cv2.COLOR_GRAY2BGR)
        return im, f, fn 

    BaseDataset.load_image = load_image_pseudo_rgb  # type: ignore[assignment] 
    BaseDataset.__kfs_gray_patched__ = True 


def _maybe_enable_grayscale_training(stage_config: Dict[str, Any]) -> bool: 
    """Enable pseudo 3-channel pipeline if config requests it. Returns True if enabled.""" 
    gray = bool(_get_cfg(stage_config, ["data", "grayscale"], False)) 
    if not gray: 
        return False 

    # 1) Make dataloader produce pseudo 3-channel grayscale images 
    _enable_ultralytics_pseudo_rgb_dataloader() 
